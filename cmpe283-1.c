 ...
 * See SDM volume 3, section 24.6.2
 */
struct capability_info second_proc_based[27] =
{
	...
	{ 24, "Intel PT Uses Guest Physical aAdresses" },
	{ 25, "Use TSC Scaling" },
 ...
 */
struct capability_info exit_based[14] =
{
 ...
 */
struct capability_info entry_based[12] =
{
	{ 2, "Load Debug Controls" },
	{ 9, "IA-32e Mode Guest" },
	{ 10, "IA-32e Mode Guest" },
...
detect_vmx_features(void)
{
	...
	pr_info("Exit based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(exit_based, 14, lo, hi);

    /* Entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry based Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(entry_based, 12, lo, hi);
}
